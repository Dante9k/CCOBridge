...
INSTALL_DIR="$AUDIT_ROOT/install"
...
cleanup() {
  ...
  if [[ "$gateway_project_dir" == "$INSTALL_DIR" ]]; then
...
INSTALL_DIR="$INSTALL_DIR" "$BUNDLE_ROOT/deploy/install.sh"
[[ "$(stat -c '%a' "$INSTALL_DIR/.env")" == "600" ]]
cp -- "$INSTALL_DIR/.env" "$ENV_SNAPSHOT"
...
INSTALL_DIR="$INSTALL_DIR" "$BUNDLE_ROOT/deploy/install.sh"
cmp --silent "$ENV_SNAPSHOT" "$INSTALL_DIR/.env"
...
printf '%s\n' \
  'Installation lifecycle passed: install, verify, repeat install, and uninstall.'

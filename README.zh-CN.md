...
版本 LiteLLM 转换，并经过一层严格的 system 内容兼容处理。项目不需要数据库、
Redis、管理后台、计费系统或外部遥测服务。
...
- 使用真正会被校验的共享 API Key，而不是 Ollama 会忽略的占位 Key；
...
| 变量 | 必需 | 默认值 | 用途 |
|---|---:|---|---|
| `CCOBRIDGE_API_KEY` | 是 | 无 | 共享 Bearer 或 `x-api-key` 凭据 |
...
CCOBridge 1.1 面向可信内网，提供共享 API Key，但不提供 TLS、个人身份、限流、
SSO 或个人级审计。必须通过主机或网络防火墙限制 4000，并且不能把无认证的 Ollama
11434 暴露给客户端。

兼容代理不会记录请求体和响应体；OpenAI 兼容流量转发到 Ollama 前会移除客户端
凭据。安装 Key 只在服务器首次安装时生成，以 `0600` 保存，不烘焙进镜像或发布包。
